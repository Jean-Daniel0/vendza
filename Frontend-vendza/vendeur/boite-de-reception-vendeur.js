...
(function () {
  ...
  async function fetchInbox(client, vendorId) {
    var resp = await client
      .from('conversations')
    ...
    conversations.forEach(function (c) {
      ...
      if (c.buyer_id) buyerIds.push(c.buyer_id);
    });
    productIds = productIds.filter(function (v, i, a) { return a.indexOf(v) === i; });
    buyerIds = buyerIds.filter(function (v, i, a) { return a.indexOf(v) === i; });
    ...
    if (buyerIds.length) {
      var b = await client.from('users').select('id, first_name, last_name, email').in('id', buyerIds);
      buyers = b.data || [];
    }
    ...
    var buyerMap = {};
    buyers.forEach(function (u) { buyerMap[u.id] = u; });

    var lastMap = await fetchLastMessages(client, conversations.map(function (c) { return c.id; }));
    var seenAt = readSeenAt(vendorId);

    return conversations.map(function (c) {
      var product = productMap[c.product_id] || {};
      var buyer = buyerMap[c.buyer_id] || {};
      var buyerName = [buyer.first_name, buyer.last_name].filter(Boolean).join(' ') || buyer.email || 'Client';
      var last = lastMap[c.id];
      var lastAt = (last && last.created_at) || c.last_message_at;
      var unread = lastAt && new Date(lastAt).getTime() > seenAt && last && last.sender_id !== vendorId;
      return {
        id: c.id,
        ...
        color: colorForId(c.buyer_id || c.id),
        initiale: initials(buyerName)
      };
    });
  }
  ...
  function filterInbox(list) {
    ...
    return list.filter(function (item) {
      ...
      if (term && item.buyer_name.toLowerCase().indexOf(term) < 0 && item.product_name.toLowerCase().indexOf(term) < 0) return false;
      return true;
    });
  }
  ...
  function renderConvList() {
    ...
    listEl.innerHTML = data.map(function (c) {
      return (
        ...
        '<div class="conv-body">' +
        '<div class="conv-top"><span class="conv-name">' + escapeHtml(c.buyer_name) + '</span><span class="conv-time">' + formatTime(c.last_message_at) + '</span></div>' +
  ...
  async function openConversation(item) {
    ...
    q('#ch-name').textContent = item.buyer_name;
    q('#ch-status').textContent = 'Client · ' + (item.product_name || 'Produit');
  ...
  async function loadMessages(client, conversationId, userId) {
    ...
    var rows = resp.data || [];
    renderMessages(client, rows, userId);
  }
  ...
  async function refreshInbox() {
    ...
    if (!user) {
      var redirect = encodeURIComponent('vendeur/boite-de-reception-vendeur.html' + (window.location.search || ''));
      window.location.href = window.VZ ? window.VZ.login('vendeur/boite-de-reception-vendeur.html') : '../authentification/connexion.html?redirect=' + redirect;
      return;
    }
  ...
  async function shareLastOrder() {
    if (!activeConversation) return;
    var client = getClient();
    if (!client) return;
    try {
      var row = null;
      var buyerCols = ['buyer_id', 'user_id', 'client_id', 'customer_id'];
      var vendorCols = ['vendor_id', 'seller_id', 'owner_id'];
      for (var vi = 0; vi < vendorCols.length && !row; vi += 1) {
        for (var bi = 0; bi < buyerCols.length; bi += 1) {
          var qry = {};
          qry[vendorCols[vi]] = currentUserId;
          qry[buyerCols[bi]] = activeConversation.buyer_id;
          var resp = await client
            .from('orders')
            .select('*')
            .eq(vendorCols[vi], currentUserId)
            .eq(buyerCols[bi], activeConversation.buyer_id)
            .order('created_at', { ascending: false })
            .limit(1);
      ...
      if (!row) {
        showToast('Aucune commande liée à ce client');
        return;
      }
      var amt = window.VendzaOrderPrices
        ? window.VendzaOrderPrices.orderTotal(row, window.VendzaOrderPrices.parseOrderItems(row))
        : Number(row.total_amount || row.total || 0);
      var api = getMsgApi();
      var text = api
        ? api.buildOrderNotificationText({
          title: 'Commande partagée',
          product: row.product_name || activeConversation.product_name || 'Produit',
          amount: amt.toLocaleString('fr-FR') + ' Gdes',
          status: row.status || 'en cours',
          orderId: row.id
        })
        : [
          'Commande partagée',
          'Produit: ' + (row.product_name || activeConversation.product_name || 'Produit'),
          'Montant: ' + amt.toLocaleString('fr-FR') + ' Gdes',
          'Statut: ' + (row.status || 'en cours'),
          'ID: ' + String(row.id || '').slice(0, 8)
        ].join(' | ');
      await sendMessage(client, activeConversation.id, currentUserId, text, null);
      await loadMessages(client, activeConversation.id, currentUserId);
      showToast('Commande partagée');
    } catch (_) {
      showToast('Impossible de partager la commande');
    }
  }

  document.addEventListener('DOMContentLoaded', function () {
    q('#sidebar-tabs').querySelectorAll('.stab').forEach(function (btn) {
      btn.addEventListener('click', function () {
        q('#sidebar-tabs').querySelectorAll('.stab').forEach(function (b) { b.classList.remove('active'); });
    ...
    var shareBtn = q('#btn-share-order');
    if (shareBtn) shareBtn.addEventListener('click', shareLastOrder);
